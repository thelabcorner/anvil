...
struct SparseToken {
    ...
    bool pnra = false;          // PROTO(pnra-cost): set only on PNRA-sourced type-3 tokens (instrumented copy; never in src/)
};
...
// ============================================================================
// PROTO(pnra-cost) — S6-2 threshold-calibration instrumentation (this file is a
// patched COPY of src/anvil.cpp; the original is never touched). Env knobs:
//   PNRA_DUMP=<path>   append one CSV row per VERIFIED PNRA candidate
//                      (verify stage), with shape, cost-model values, commit
//                      decision, and (when PNRA_CF=1) the measured real
//                      marginal payload delta of each committed candidate.
//   PNRA_MODE=0        legacy: commit iff pnra_c < alt_c  (exact EXP. X clone)
//   PNRA_MODE=1        margin family: commit iff (alt_c - pnra_c) >= M(shape)
//     PNRA_FORM=0:  M = G0 + G1*varint_bytes(D-1) + G2*[single-isolated-field]
//     PNRA_FORM=1:  M = GAMMA * raw_framing_bits(L,D,F,R)
//   PNRA_CF=1          measure per-committed-candidate real counterfactual
//                      delta (re-serialize block with the candidate replaced
//                      by its exact+literal alternative; spans are identical,
//                      so the rest of the parse is unaffected).
// All defaults reproduce EXP. X behavior bit-for-bit.
struct ProtoRec {
    uint32_t block, pos, len, dist;
    uint32_t n_tfo, n_res;
    uint32_t exact_len, exact_dist;
    double pnra_c, alt_c;         // bits (existing shared formula)
    int committed;                // 1 = committed under the ACTIVE rule
    double real_d_bytes;          // counterfactual payload delta (committed cands; NaN otherwise)
};
static std::vector<ProtoRec> g_proto_recs;
static uint32_t g_proto_block = 0;
static int    g_proto_mode  = 0;
static int    g_proto_form  = 0;
static double g_g0 = 0.0, g_g1 = 0.0, g_g2 = 0.0, g_gamma = 0.0;
static bool   g_proto_cf = false;
static std::string g_proto_dump_path;

static inline uint32_t proto_varint_bytes(uint64_t x) {
    uint32_t b = 1; while (x >= 128) { x >>= 7; ++b; }
    return b;
}
// Raw (pre-entropy) framing bits of a type-3 token: type byte + ml varint +
// ds varint + residual-mask words + transform-mask words + residual bytes.
// This is the per-token fixed wire overhead the EXP. X root cause names.
static inline double proto_framing_raw_bits(uint32_t L, uint32_t D, uint32_t F, uint32_t R) {
    const uint32_t mask_bits = 32u * ((L + 31u) / 32u);
    const uint32_t tmask_bits = 32u * (((L / 4u) + 31u) / 32u);
    return 8.0 + 8.0 * proto_varint_bytes(L - 4) + 8.0 * proto_varint_bytes(D - 1)
         + mask_bits + tmask_bits + 8.0 * R;
}
static void proto_init_env() {
    if (const char* e = getenv("PNRA_MODE"))  g_proto_mode = atoi(e);
    if (const char* e = getenv("PNRA_FORM"))  g_proto_form = atoi(e);
    if (const char* e = getenv("PNRA_G0"))    g_g0 = atof(e);
    if (const char* e = getenv("PNRA_G1"))    g_g1 = atof(e);
    if (const char* e = getenv("PNRA_G2"))    g_g2 = atof(e);
    if (const char* e = getenv("PNRA_GAMMA")) g_gamma = atof(e);
    if (const char* e = getenv("PNRA_CF"))    g_proto_cf = atoi(e) != 0;
    if (const char* e = getenv("PNRA_DUMP"))  g_proto_dump_path = e;
}
static void proto_dump_csv() {
    if (g_proto_dump_path.empty()) return;
    FILE* f = fopen(g_proto_dump_path.c_str(), "w");
    if (!f) { fprintf(stderr, "PROTO: cannot open dump %s\n", g_proto_dump_path.c_str()); return; }
    fprintf(f, "block,pos,len,dist,n_tfo,n_res,exact_len,exact_dist,pnra_c,alt_c,S,committed,real_d_bytes\n");
    for (const auto& r : g_proto_recs) {
        fprintf(f, "%u,%u,%u,%u,%u,%u,%u,%u,%.4f,%.4f,%.4f,%d,%.6f\n",
                r.block, r.pos, r.len, r.dist, r.n_tfo, r.n_res,
                r.exact_len, r.exact_dist, r.pnra_c, r.alt_c,
                r.alt_c - r.pnra_c, r.committed, r.real_d_bytes);
    }
    fclose(f);
    fprintf(stderr, "PROTO: dumped %zu candidate rows to %s\n", g_proto_recs.size(), g_proto_dump_path.c_str());
}
...
static std::vector<SparseToken> parse_sparse(const std::vector<uint8_t>& d, uint32_t max_chain, uint32_t max_match,
                                             ...
                                             bool pnra=false) {
    const uint32_t n=static_cast<uint32_t>(d.size());
    ++g_proto_block; // PROTO(pnra-cost): block tag for candidate records
                ...
                if (ub != v.begin()) {
                    ...
                    if (q < i && (i - q) >= 4) {
                        ...
                        if (mf.find_pnra_at(i, q, pm, litcost, avg_lit, work, dead_band)) {
                            ...
                            // PROTO(pnra-cost): record EVERY verified candidate, then apply the
                            // ACTIVE acceptance rule. MODE 0 reproduces EXP. X exactly.
                            {
                                ProtoRec r{};
                                r.block=g_proto_block; r.pos=i; r.len=pm.len; r.dist=pm.dist;
                                r.n_tfo=(uint32_t)pm.tfo.size(); r.n_res=(uint32_t)pm.off.size();
                                r.exact_len=(exact.len>=4)?exact.len:0u; r.exact_dist=(exact.len>=4)?exact.dist:0u;
                                r.pnra_c=pnra_c; r.alt_c=alt_c; r.real_d_bytes=std::nan("");
                                bool commit;
                                if (g_proto_mode == 0) {
                                    commit = pnra_c < alt_c;
                                } else {
                                    const double S = alt_c - pnra_c;
                                    double M;
                                    if (g_proto_form == 0) {
                                        bool single_iso = (pm.tfo.size()==1 && pm.off.empty() && pm.len<=8);
                                        M = g_g0 + g_g1 * proto_varint_bytes(pm.dist-1) + g_g2 * (single_iso?1.0:0.0);
                                    } else {
                                        M = g_gamma * proto_framing_raw_bits(pm.len, pm.dist,
                                                                             (uint32_t)pm.tfo.size(), (uint32_t)pm.off.size());
                                    }
                                    commit = S >= M;
                                }
                                r.committed = commit ? 1 : 0;
                                g_proto_recs.push_back(r);
                                if (!commit) goto pnra_done; // fall through to ordinary candidates
                            }
                            if (pnra_c < alt_c || g_proto_mode != 0) {
                                ++g_pnra_commit;
...
static std::vector<uint8_t> encode_tokens_tcopy(const std::vector<uint8_t>& d, const std::vector<SparseToken>& toks) {
    ...
    // ------------------------------------------------------------------
    // PROTO(pnra-cost): real per-candidate counterfactual cost. A PNRA type-3
    // token and its alternative (capped exact match + literal run) cover the
    // SAME output span, so swapping one for the other leaves every other parse
    // decision unchanged — re-serializing the block with the swap isolates the
    // TRUE marginal payload cost of that single commit, entropy streams
    // included. This is the ground truth the shared formula misprices (EXP. X).
    static bool in_cf = false; // recursion guard: CF re-entry sees cf disabled
    if (g_proto_cf && !in_cf) {
        const uint32_t blk = g_proto_block;
        const size_t base = out.size();
        for (size_t j = 0; j < toks.size(); ++j) {
            const auto& t = toks[j];
            if (!(t.type == 3 && t.pnra)) continue;
            ProtoRec* rec = nullptr;
            for (auto& r : g_proto_recs)
                if (r.block == blk && r.pos == t.pos && r.committed == 1) { rec = &r; break; }
            if (!rec) continue;
            std::vector<SparseToken> alt;
            alt.reserve(toks.size() + 1);
            for (size_t k = 0; k < toks.size(); ++k) {
                if (k == j) {
                    const uint32_t el = std::min(rec->exact_len, t.len);
                    if (el >= 4)
                        alt.push_back(SparseToken{1u, t.pos, el, rec->exact_dist, {}, {}, {}, false});
                    const uint32_t litlen = t.len - el;
                    if (litlen > 0)
                        alt.push_back(SparseToken{0u, t.pos + el, litlen, 0u, {}, {}, {}, false});
                } else {
                    alt.push_back(toks[k]);
                }
            }
            // merge adjacent literal runs (the greedy parser would have emitted one run)
            std::vector<SparseToken> merged;
            merged.reserve(alt.size());
            for (auto& s : alt) {
                if (!merged.empty() && merged.back().type == 0 && s.type == 0 &&
                    merged.back().pos + merged.back().len == s.pos)
                    merged.back().len += s.len;
                else
                    merged.push_back(s);
            }
            in_cf = true;
            const size_t sz_alt = encode_tokens_tcopy(d, merged).size();
            in_cf = false;
            rec->real_d_bytes = (double)sz_alt - (double)base;
        }
    }
    return out;
}
...
int main(int argc,char**argv) {
    ...
    proto_init_env();                       // PROTO(pnra-cost)
    if (!g_proto_dump_path.empty()) atexit(proto_dump_csv); // PROTO(pnra-cost)
    try {
