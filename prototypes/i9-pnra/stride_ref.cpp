// stride_ref.cpp — prototypes/i9-pnra (pnra lane, iteration 9)
//
// NATIVE PER-REGION STEP REFERENCE — a parser-feature prototype, not a global
// pre-LZ transform.
//
...
static Encoded encode(const std::vector<u8>& raw, int backend, int pmode, int bq, bool force_literal) {
    ...
    put32(o, kMagic); o.push_back(1); o.push_back(u8(backend)); o.push_back(u8(pmode)); o.push_back(0);
...
static std::vector<u8> decode(const std::vector<u8>& b) {
    ...
    for (u32 r = 0; r < nr; r++) {
        u64 gap, len, type;
        if (!getvar(par, par_len, ppos, gap) || !getvar(par, par_len, ppos, len) || !getvar(par, par_len, ppos, type)) {
            std::fprintf(stderr, "params corrupt\n"); std::exit(1);
        }
        Region g; g.start = (u32)(prev_end + gap); g.len = (u32)len; g.type = (u8)type;
        if (g.type == 0) {
            u64 pv;
...
struct Opts { int backend = 0, pmode = 1, bq = 11, reps = 7; bool literal = false; std::string in, out; };
...
// ---------------------------------------------------------------- selftest
static bool roundtrip_check(const std::vector<u8>& raw, int backend, int pmode, bool literal, const char* tag) {
    Encoded e = encode(raw, backend, pmode, 11, literal);
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::fprintf(stderr, "usage: stride_ref selftest | c <in> <out> [opts] | d <in> <out> | bench <in> [opts]\n");
        return 2;
    }
    ...
    if (mode == "c") {
        ...
        auto t0 = now_us();
        Encoded e = encode(raw, o.backend, o.pmode, o.bq, o.literal);
        auto t1 = now_us();
    ...
    if (mode == "bench") {
        ...
        for (int i = 0; i < o.reps; i++) {
            auto t0 = now_us(); Encoded e2 = encode(raw, o.backend, o.pmode, o.bq, o.literal); auto t1 = now_us();
